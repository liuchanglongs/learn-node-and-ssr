...
1. 方式一：`捕获所有不匹配的路由`（即 404 not found 页面）

- `通过在方括号内添加三个点` ，如：[...xxx].tsx 语法，如在其它目录下的话，仅作用于该目录以及子目录

- 比如：访问 pages/post/[...slug].js 路径，将匹配/post/a 、/post/a/b、/post/a/b/c 等，但不匹配 /post

  > 注意：是可以使用除了 slug 以外的名称，例如：[...param].tsx。
  > **三个点的配置规则是以当前目录为跟路由，作用于该目录以及子目录没有匹配上的路由**

- [...slug] 匹配的参数将作为查询参数发送到页面，并且它始终是一个数组
  ✓ 如：/pages/register/[...slug].tsx ,访问 /register/llll/oooo/oooppp?lcl=lcl，对应的参数为：
  ```js
  {
    "lcl": "lcl",
    "slug": [
...
2. 方式二（推荐）：在 pages 根目录新建 404.tsx 页面（注意：只支持根目录）- 当然还支持 500.tsx 文件，即客户端或者服务器端报错

# 路由匹配规则

路由匹配优先级， 即`预定义路由优先于动态路由;动态路由优先于捕获所有路由`。
请看以下示例：[pages/routerRule](pages/routerRule)
![alt text](img/路由匹配规则.png)

1. 预定义路由：/routerRule/create
   ✓ 将匹配 /routerRule/create.tsx
2. 动态路由 ：/routerRule/c
   ✓ 将匹配 /routerRule/[id].tsx
3. 捕获所有路由：/routerRule/c/40
   ✓ 将匹配 /routerRule[...404].tsx
